...
export default {
  ...
  computed: {
    ...
    todoList() {
      ...
      if (this.$store.getters.getTodoList.length < this.todoNum) {
        this.goList(this.$store.getters.getTodoList[0]._id);
      }
      this.todoNum = number;
      return this.$store.getters.getTodoList; // 返回vuex getters.js 定义的getTodoList数据
    },
  ...
  watch: {
    todoId(_id) {
      this.$router.push({ name: "todo", params: { _id: _id } });
      ...
      addTodo({}).then((data) => {
        ...
        this.$store.dispatch("getTodo").then(() => {
          this.$nextTick(() => {
            setTimeout(() => {
              // 打开新建的
              this.goList(this.todoList[this.todoList.length - 1]._id);
            }, 100);
            // 然后watch会被触发
          });
        });
